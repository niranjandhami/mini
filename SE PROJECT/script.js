...
if (window.location.pathname.includes('dashboard.html')) {
  ...
  function showCourses(courses, categoryName) {
    ...
    courseList.innerHTML = `<h4>${categoryName}</h4>`;
    ...
    courses.forEach(course => {
      ...
      courseEl.innerHTML = `
        <div>
          <h5>${course.name}</h5>
          <small>${course.mode} • ${course.duration}</small>
        </div>
        <button onclick="startExamRegistration('${course.id}')" class="btn">Register Exam</button>
      `;
